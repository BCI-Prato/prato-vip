...
ALTER TABLE public.packages ADD COLUMN IF NOT EXISTS features text[] NOT NULL DEFAULT '{}';

-- Copy existing bonuses into features when features is empty
UPDATE public.packages
SET features = bonuses
WHERE (features IS NULL OR array_length(features,1) IS NULL)
  AND bonuses IS NOT NULL AND array_length(bonuses,1) IS NOT NULL;

-- Update RPC to use 24h-ahead rule and enforce min 3 or 0
...
BEGIN
  ...
  LOOP
    ...
    -- Regra: bloqueio com 24h de antecedência da entrega
  ...
  -- Estornos (refunds) quando a quantidade diminui ou é zerada
