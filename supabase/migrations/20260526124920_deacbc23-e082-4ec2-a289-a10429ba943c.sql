...
-- 1. Remove user INSERT policy on credit_transactions (privilege escalation)
DROP POLICY IF EXISTS "Users insert own consumption/refund" ON public.credit_transactions;

-- 2. Make confirm_scheduled_deliveries SECURITY DEFINER so it can still write credit_transactions
...
-- 3. support_tickets: prevent users from changing admin-only fields
CREATE OR REPLACE FUNCTION public.prevent_admin_field_changes_on_tickets()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF NOT has_role(auth.uid(), 'admin'::app_role) THEN
    IF NEW.admin_reply IS DISTINCT FROM OLD.admin_reply
      ...
      RAISE EXCEPTION 'Não é permitido alterar campos administrativos do ticket';
    END IF;
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_prevent_admin_field_changes_on_tickets ON public.support_tickets;
CREATE TRIGGER trg_prevent_admin_field_changes_on_tickets
BEFORE UPDATE ON public.support_tickets
FOR EACH ROW EXECUTE FUNCTION public.prevent_admin_field_changes_on_tickets();
...
REVOKE EXECUTE ON FUNCTION public.prevent_admin_field_changes_on_tickets() FROM PUBLIC, anon, authenticated;

-- Keep has_role, cnpj_exists, confirm_scheduled_deliveries callable
...
-- 5. Realtime: restrict subscriptions to user's own credit topic
-- Enable RLS on realtime.messages and add a policy scoping topics to the user's id
ALTER TABLE IF EXISTS realtime.messages ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users subscribe to own client_credits topic" ON realtime.messages;
CREATE POLICY "Users subscribe to own client_credits topic"
ON realtime.messages
FOR SELECT
TO authenticated
USING (
  (realtime.topic() = ('client_credits:' || auth.uid()::text))
